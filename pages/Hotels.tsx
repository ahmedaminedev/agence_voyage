...
const Hotels: React.FC = () => {
  ...
  const [filters, setFilters] = useState({
    destinations: currentPage.params?.destination ? new Set([currentPage.params.destination]) : new Set<string>(),
  ...
  const staysSource = useMemo(() => {
    let source = allStays.filter(s => s.type === 'hotel' || s.type === 'promo');
    
    if (currentPage.params?.amenities) {
        const requiredAmenity = currentPage.params.amenities === 'waterpark' ? 'Piscine' : currentPage.params.amenities;
        source = source.filter(s => s.amenities?.some(a => a.toLowerCase().includes(requiredAmenity.toLowerCase())));
    }
    if (currentPage.params?.category) {
        if(currentPage.params.category === 'luxury'){
            source = source.filter(s => s.rating >= 4.8);
        }
        if(currentPage.params.category === 'beach'){
            source = source.filter(s => s.mainAmenities?.includes('Plage'));
        }
    }
    if (currentPage.params?.chain) {
        const chainName = currentPage.params.chain.replace('-', ' ');
        source = source.filter(s => s.title.toLowerCase().includes(chainName));
    }
  ...
  const pageTitle = currentPage.params?.destination ? `Hôtels à ${currentPage.params.destination}` : 'Hôtels en Tunisie';
  ...
  return (
    <div className="min-h-screen">
       <div className="bg-cover bg-center py-20" style={{backgroundImage: "url('https://picsum.photos/seed/hotels-hero/1920/400')"}}>
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center bg-black/30 backdrop-blur-sm py-8 rounded-xl">
                ...
                <p className="text-lg text-white mt-2 drop-shadow-md">Affinez votre recherche pour trouver l'hôtel parfait.</p>
            </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="lg:w-1/4">
            <div className="sticky top-24 bg-white p-6 rounded-xl shadow-lg border border-gray-100 space-y-6">
              ...
              <hr/>
              <FilterCheckboxes title="Types" options={availableTypes} selected={filters.types} onChange={(val) => handleCheckboxChange('types', val)} />
              <hr/>
              <div>
          ...
          <main className="lg:w-3/4">
            <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-lg shadow-sm border flex-wrap gap-4">
              <p className="text-sm font-medium text-gray-600">
                {filteredAndSortedStays.length} hôtel{filteredAndSortedStays.length > 1 ? 's' : ''} trouvé{filteredAndSortedStays.length > 1 ? 's' : ''}
              </p>
            ...
            {filteredAndSortedStays.length > 0 ? (
                ...
                <div className="text-center py-16 bg-white rounded-lg shadow-md flex flex-col items-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-800">Aucun hôtel ne correspond à vos critères</h3>
